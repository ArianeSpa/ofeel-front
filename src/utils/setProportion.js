export default (goal, cal_jour) => {
    ...
    //je calcule dans les 3 fonctions suivantes les proportions de glucides, protéines et lipides qu'il me faut répartir dans ma journée
    //pour les protéines
    ...
    const setPropGlu = (goal) => {
        if(goal == 'perte-de-poids') {
            return 0.33;
        } else if (goal == 'prise-de-masse' || goal == 'remise-en-forme') {
            return 0.488;
        }
    }
    ...
    //on calcule les valeurs caloriques par repas, sachant que le petit déjeuner et le diner ont la même valeur.
    const cal_p_dej_din = Math.round(cal_obj * 0.3);
    const cal_dej = Math.round(cal_obj * 0.4);

    //on sait quelle proportion de glucides, protéines et lipides on veut pour chaque repas. Ce qui nous intéresse maintenant c'est l'équivalent en grammes, sachant que 1g de protéine ou de glucides vaut 4kcal et que 1g de lipides vaut 9kcal.
    ...
    const propObject = { cal_dej, cal_obj, cal_p_dej_din, prop_glu, prop_lip, prop_prot, q_glu_dej, q_glu_p_dej_din, q_lip_dej, q_lip_p_dej_din, q_prot_dej, q_prot_p_dej_din };
